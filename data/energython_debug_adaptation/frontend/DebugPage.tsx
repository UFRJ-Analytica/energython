...
const JUPYTERLITE_URL =
  (import.meta.env.VITE_JUPYTERLITE_URL as string | undefined) ??
...
export default function DebugPage() {
  ...
  const lab = useDebugLabTemplate(activeId, 30, 200)
      ...
      <main className="container mx-auto max-w-7xl space-y-8 px-6 py-8">
        ...
        <Tabs defaultValue="tower" className="space-y-5">
          <TabsList className="flex h-auto w-full flex-wrap justify-start bg-white/10 p-1 text-white">
            ...
            <TabsTrigger value="lab">7. Analyst Lab</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="unidades">
            <Card className="border-white/10 bg-white/[0.04] text-white">
              <CardHeader>
                <CardTitle>Unidades individuais / granularidade</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <p className="text-sm text-slate-400">
                  {String(unidades.data?.metadata?.observacao ?? "Aba preparada para conectar sk_usina real via dw.fato_restricao_coff.")}
                </p>
              ...
              <CardContent className="space-y-3 p-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="flex items-center gap-2 text-sm text-slate-300">
                    <Badge variant="outline">Analyst Lab · JupyterLite</Badge>
                    <span className="text-xs text-slate-500">{lab.data?.notebook_name ?? activeId}</span>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <Button asChild size="sm" variant="outline">
                      <a href={`/api/debug/usinas/${encodeURIComponent(activeId)}/lab-template/notebook`} target="_blank" rel="noreferrer">
                        ⬇️ notebook .ipynb
                      </a>
                    </Button>
                    <Button asChild size="sm" variant="outline">
                      <a href={`/api/debug/usinas/${encodeURIComponent(activeId)}/lab-template/csv`} target="_blank" rel="noreferrer">
                        ⬇️ CSV
                      </a>
                    </Button>
                    <Button asChild size="sm" variant="outline">
                      <a href={JUPYTERLITE_URL} target="_blank" rel="noreferrer">
                        <ExternalLink className="mr-1 h-3.5 w-3.5" /> nova aba
                      </a>
                    </Button>
                  </div>
                </div>
                ...
                <div className="overflow-hidden rounded-lg border border-white/10 bg-black/40">
                  <iframe
                    ...
                    src={JUPYTERLITE_URL}
                    className="w-full"
                    style={{ border: "0", height: "calc(100vh - 150px)", minHeight: "640px" }}
                  />
                </div>
              </CardContent>
            </Card>
